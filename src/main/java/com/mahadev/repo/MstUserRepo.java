...
public interface MstUserRepo extends JpaRepository<MstUserBo, Integer> {
    ...
    @Modifying
	@Query("update MstUserBo set full_name=?1,mobile_no=?2,address=?3,city=?4,pincode=?5,capital=?6,percentage=?7,adharcard=?8,check=?9,lstUpdDate=?10,lstUpdIp=?11,lstUpdUser=?12 where category_id=?13")
	int updateMstGpImageGallery(String full_name, String mobile_no, String address, String city, String pincode,
			String capital, String percentage, String adharcard, String check, Date lstUpdDate, String lstUpdIp,
